...
def load_data(file_path):
    '''
        argument:
            file_path: Path to OntoOmicsKG data directory (parent directory)
    ...
    # Check for OntoOmicsKG format (TSV files)
    entity_tsv = os.path.join(file_path, 'edges.filtered.tsv.entities.tsv')
    relation_tsv = os.path.join(file_path, 'edges.filtered.tsv.relations.tsv')

    if os.path.exists(entity_tsv) and os.path.exists(relation_tsv):
        # OntoOmicsKG format: TSV files with id \t name format
        entity2id = {}
        with open(entity_tsv) as f:
            for line in f:
                eid, entity = line.strip().split('\t')
                entity2id[entity] = int(eid)
        
        relation2id = {}
        with open(relation_tsv) as f:
            for line in f:
        ...
        raise FileNotFoundError("Could not find OntoOmicsKG data files. Expected:\n"
                              ...
                              "  - edges.filtered.tsv.relations.tsv\n"
                              "  - edges_train.tsv\n"
                              "  - edges_val.tsv\n"
                              "  - edges_test.tsv")
