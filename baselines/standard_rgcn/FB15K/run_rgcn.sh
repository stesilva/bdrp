...
# Check for dataset
echo "Checking for FB15k-237 dataset..."
if [ -d "$SLURM_SUBMIT_DIR/data/FB15k-237" ]; then
    echo "Found: data/FB15k-237 directory"
    echo "Dataset files:"
    ls -lh "$SLURM_SUBMIT_DIR/data/FB15k-237/"*.dict "$SLURM_SUBMIT_DIR/data/FB15k-237/"*.txt 2>/dev/null | head -5 || echo "Warning: Expected files not found"
    echo ""
    echo "Required files:"
    echo "  - entities.dict"
    echo "  - relations.dict"
    ...
    echo "ERROR: data/FB15k-237 directory not found"
    echo "Please ensure the dataset is in: $SLURM_SUBMIT_DIR/data/FB15k-237/"
    echo "Expected structure:"
    echo "  data/FB15k-237/entities.dict"
    echo "  data/FB15k-237/relations.dict"
    echo "  data/FB15k-237/train.txt"
    echo "  data/FB15k-237/valid.txt"
    echo "  data/FB15k-237/test.txt"
    exit 1
fi
echo ""
...
echo "Test graph size:   200000 triplets (default, use --test-graph-size to change)"
...
echo "      To use all triplets, set --test-graph-size -1"
echo ""
...
# Default uses 200000 triplets for test graph to avoid OOM
# Adjust --test-graph-size if needed:
#   - 150000: safer, uses less memory
#   - 200000: default, balanced
#   - -1: use all triplets (may cause OOM)
python main.py --gpu 0 --test-graph-size 200000 2>&1 || {
    EXIT_STATUS=$?
    echo ""
    echo "======================================================================"
