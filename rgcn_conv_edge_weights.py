...
class RGCNConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._use_segment_matmul_heuristic_output: torch.jit.Attribute(None, Optional[float])
    ...
    def forward(self, x: Union[OptTensor, Tuple[OptTensor, Tensor]],
        ...
        Args:
            ...
            edge_weight (torch.Tensor, optional): scalar edge attribute for normalization and weighting.
        """
        ...
        # Normalize edge weights: for each edge (j->i) and relation r,
        # a_ij = edge_weight / sum_{k in N_r(i)} edge_weight
        if edge_weight is not None:
            dst_nodes = edge_index[1]
            key = dst_nodes * self.num_relations + edge_type  # Per target node and relation
            norm_denom = scatter(edge_weight, key, dim_size=x_r.size(0)*self.num_relations, reduce='sum')
            normed_weights = edge_weight / (norm_denom[key] + 1e-8)  # assign denominator to edge order
        else:
            normed_weights = None
        ...
        if self.num_blocks is not None:  # Block-diagonal-decomposition =====
            ...
            for i in range(self.num_relations):
                ...
                normed_weight_i = normed_weights[mask] if normed_weights is not None else None
                h = self.propagate(tmp, x=x_l, edge_type_ptr=None, edge_weight=normed_weight_i, size=size)
            ...
            if (use_segment_matmul and torch_geometric.typing.WITH_SEGMM
                ...
                if not self.is_sorted:
                    if (edge_type[1:] < edge_type[:-1]).any():
                        ...
                        if edge_weight is not None:
                            normed_weights = normed_weights[perm]  # reorder normalized weights
                edge_type_ptr = index2ptr(edge_type, self.num_relations)
                out = self.propagate(edge_index, x=x_l,
                                    edge_type_ptr=edge_type_ptr,
                                    edge_weight=normed_weights, size=size)
            else:
                for i in range(self.num_relations):
                    mask = (edge_type == i)
                    tmp = masked_edge_index(edge_index, mask)
