...
def build_test_graph(num_nodes, num_rels, triplets, confidence_scores=None, max_edges=None):
    ...
    # Compute degree-based normalization
    degree_norm = edge_normalization(edge_type, edge_index, num_nodes, num_rels)
    
    # Combine confidence with degree normalization (if confidence available)
    if graph_confidence is not None:
        # Option: Multiply confidence with degree normalization
        data.edge_weight = graph_confidence * degree_norm
    else:
        # Use degree normalization only
        data.edge_weight = degree_norm
    
    # Keep edge_norm for backward compatibility
    data.edge_norm = degree_norm
